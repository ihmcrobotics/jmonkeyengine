...
public class InputShared
{
    ...
    */
   public static InputShared create(Semantic semantic, Integer offset, Integer stride, Integer set, String[] idref, String[] name, boolean[] bool,
                                    float[] floats, int[] ints)
   {
      InputShared instance = new InputShared(semantic, (offset == null) ? 0 : offset, (stride == null) ? 1 : stride, (set == null) ? 0 : set, idref, name,
                                bool, floats, ints);
   ...
   public void transferData(int chunkSize, int[] chunks, PolygonData[] polygons)
   {
      ...
      switch (semantic)
      {
         ...
         case POSITION :
            if (floatBuffer != null)
               transferVertices(indices, polygons, floatBuffer);

            break;

         case NORMAL :
            if (floatBuffer != null)
               transferNormals(indices, polygons, floatBuffer);

            break;

         case BINORMAL :
            if (floatBuffer != null)
               transferBinormals(indices, polygons, floatBuffer);

            break;

         case TEXCOORD :
            if (floatBuffer != null)
               transferTexCoords(indices, polygons, floatBuffer);

            break;

         case TANGENT :
            if (floatBuffer != null)
               transferTangents(indices, polygons, floatBuffer);
